...
namespace DiGraph
{
    class Graph
    {
        ...
        public int RunDijkstra(int source, int destination)//runs dijkstras algorithm on the adjacency matrix
        {
            ...
            //enqueue the source
            pq.Enqueue(source, distance[source]);
            ...
            while (!pq.empty())
            {
                int current_node = pq.dequeue_min();
                foreach (int neighbour in ListNeighbours(current_node))//scan each row fully
                {
                    
                    int alt = distance[current_node] + adjMatrix[current_node, neighbour];
                    if (alt < distance[neighbour])
                    {
                        distance[neighbour] = alt;
                        previous[neighbour] = current_node;
                        pq.Enqueue(current_node, distance[neighbour]);
                    }
                }
